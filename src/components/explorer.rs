...
impl AppDM {
    pub(crate) fn ui_explorer(&mut self, ui: &mut egui::Ui) {
                ...
                if !self.project.dem.dem.x.is_empty() {
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ...
                            if ui
                                .button(Self::get_display_icon(
                                    true,
                                    !self.is_viewer_properties,
                                    self.project.dem.section_surface,
                                ))
                                .clicked()
                            {
                                self.project.dem.section_surface =
        ...
        egui::CollapsingHeader::new(
            "Calibration data (".to_string() + nb_sat.to_string().as_str() + ")",
        )
        .default_open(true)
        .show(ui, |ui| {
            for k in 0..nb_sat {
                ...
                ui.push_id(k, |ui| {
                    ...
                    egui::CollapsingHeader::new(bundle_name)
                        ...
                        .show(ui, |ui| {
                            ui.label(format!(
                                "az: {}° ; i: {}°",
                                rad2deg(bundle.sar_geometry.azimuth),
                                rad2deg(bundle.sar_geometry.incidence)
                            ));
